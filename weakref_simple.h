...
struct ref *get_ref(void *_Atomic *weakref) {
  ...
  while ((ref = atomic_fetch_or(_weakref, 1)) & 1) (void)0;
  if (ref && ++((struct ref *)ref)->count == -1) *(volatile char *)0 = 0;
  uintptr_t locked_ref = ref | 1;
  if (!atomic_compare_exchange_strong(_weakref, &locked_ref, ref))
    ref && ((struct ref *)ref)->count--;
